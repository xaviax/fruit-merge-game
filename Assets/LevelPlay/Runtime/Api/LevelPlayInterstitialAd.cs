...
{
    public sealed class LevelPlayInterstitialAd : ILevelPlayInterstitialAd
    {
        /// <summary>
        ...
        public LevelPlayInterstitialAd(string adUnitId, Config config = null)
        {
...
#elif UNITY_ANDROID
            m_InterstitialAd = new AndroidInterstitialAd(adUnitId, config?.PlatformConfig as AndroidInterstitialAd.Config);
#elif UNITY_IOS
            m_InterstitialAd = new IosInterstitialAd(adUnitId, config?.PlatformConfig as IosInterstitialAd.Config);
#else
        ...
        private void SetupEvents()
        {
            ...
            m_InterstitialAd.OnAdDisplayFailed += (info, error) => OnAdDisplayFailed?.Invoke(info, error);
            m_InterstitialAd.OnAdInfoChanged += (info) => OnAdInfoChanged?.Invoke(info);
            m_InterstitialAd.OnAdImpressionDataReady += (data) => OnAdImpressionDataReady?.Invoke(data);
        }
