...
{
    class LevelPlayNetworkManager : ILevelPlayNetworkManager
    {
        const string k_SdkVersionsUrl = "https://s3.amazonaws.com/ssa.public/Unity/Package/V2/LevelPlayVersions.json";
        const string k_LevelPlayPackageVersion = Constants.k_PackageVersion;
        ...
        readonly VersionComparer m_VersionComparer = new VersionComparer();
        readonly string m_LevelPlayPackageVersion;
        readonly IFileService m_FileService;
        readonly IAssetDatabaseService m_AssetDatabaseService;
        readonly IXmlDocumentFactory m_XmlDocumentFactory;
        readonly IWebRequestService m_WebRequestService;
        readonly ILevelPlayLogger m_LevelPlayLogger;
        ...
        internal LevelPlayNetworkManager(
            ...
            IAssetDatabaseService assetDataBaseService,
        ...
        )
        {
            ...
            m_AssetDatabaseService = assetDataBaseService;
        ...
        public IEnumerable<SdkVersion> CompatibleIronSourceSdkVersions()
        {
            ...
            if (specificVersionMatches.Count == 1)
            {
        ...
        public void Uninstall(Adapter adapter)
        {
            try
            {
                m_AssetDatabaseService.DeleteAsset(adapter.GetDependencyXmlPath());
            }
            catch (Exception ex)
            {
                m_LevelPlayLogger.LogError($"Error deleting {adapter.DisplayName} adapter. To manually delete, find and delete {adapter.DependencyXmlFileName} from your project.\nException : {ex.Message}");
            }
        }
        ...
        public async Task Install(UnityPackageVersion unityPackageVersion)
        {
            ...
            {
                var url = this.UnityPackage.BuildPackageUrl(unityPackageVersion.Version);
                var webRequest = m_WebRequestService.Get(url);
                webRequest.SendWebRequest();
        ...
        IEnumerable<AdapterVersion> GetVersionsOrderedLargestToSmallest(IEnumerable<AdapterVersion> versions)
        {
            return versions.OrderByDescending(version => version.Version, this.m_VersionComparer).AsEnumerable();
        }

        IEnumerable<UnityPackageVersion> GetVersionsOrderedLargestToSmallest(IEnumerable<UnityPackageVersion> versions)
        {
            return versions.OrderByDescending(version => version.Version, this.m_VersionComparer).AsEnumerable();
        }

        IEnumerable<SdkVersion> GetVersionsOrderedLargestToSmallest(IEnumerable<SdkVersion> versions)
        {
            return versions.OrderByDescending(version => version.Version, this.m_VersionComparer).AsEnumerable();
        }
    }
